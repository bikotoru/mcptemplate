...
class CRUDGenerator:
    ...
    def __init__(self, templates_path: Optional[str] = None):
        ...
        # Configurar Jinja2 (equivalente a Handlebars en TypeScript)
    ...
    def _register_helpers(self) -> None:
        """Registra helpers personalizados para Jinja2 (equivalente a Handlebars helpers)"""
        # Helper para comparación (equivalente a eq, neq, and, or, not de Handlebars)
        self.jinja_env.filters['eq'] = lambda a, b: a == b
        self.jinja_env.filters['neq'] = lambda a, b: a != b
        self.jinja_env.filters['and'] = lambda a, b: a and b
        self.jinja_env.filters['or'] = lambda a, b: a or b
        self.jinja_env.filters['not'] = lambda a: not a
        
        # Helper para transformaciones de string (equivalente a los de TypeScript)
        ...
        # Helper para arrays (equivalente a includes, some de Handlebars)
        ...
        # Helper para JSON (equivalente a json, JSON.stringify de Handlebars)
        self.jinja_env.filters['json'] = lambda obj: str(obj)
        self.jinja_env.filters['tojson'] = lambda obj: str(obj)
        
        # Helper para obtener tipo TypeScript (equivalente a getTypeScriptType de Handlebars)
        self.jinja_env.filters['getTypeScriptType'] = self._get_typescript_type
        
        # Helper para comentarios (equivalente a comment de Handlebars)
        self.jinja_env.filters['comment'] = lambda text, style='//': '\n'.join(f"{style} {line}" for line in text.split('\n'))
        
        # Helper para primer elemento (equivalente a first de Handlebars)
        ...
        self.jinja_env.filters['firstOf'] = lambda obj, prop: obj[prop][0] if isinstance(obj.get(prop), list) and obj.get(prop) else ''
        
        # Helpers para condicionales de posición (equivalente a ifFirst, ifLast de Handlebars)
        def if_first(index):
            return index == 0
        
        def if_last(index, array_length):
            return index == array_length - 1
        
        self.jinja_env.globals['ifFirst'] = if_first
        self.jinja_env.globals['ifLast'] = if_last
    
    def _get_typescript_type(self, field_type: str) -> str:
        """Obtiene tipo TypeScript desde tipo de campo (equivalente a getTypeScriptType de Handlebars)"""
        type_mapping = {
            'text': 'string',
            'email': 'string',
            'password': 'string',
    ...
    async def generate(self, config: CRUDGeneratorConfig, options: GeneratorOptions = None) -> GenerationResult:
        """Genera un módulo CRUD completo (equivalente al método generate de TypeScript)"""
        if options is None:
    ...
    def _create_template_context(self, config: CRUDGeneratorConfig) -> TemplateContext:
        """Crea el contexto para los templates (equivalente a createTemplateContext de TypeScript)"""
    ...
    async def _find_template_files(self) -> List[str]:
        """Busca todos los archivos de template (equivalente a findTemplateFiles de TypeScript)"""
        patterns = [
            '**/*.template',
            '**/*.tsx.template',
            '**/*.ts.template',
    ...
    async def _process_template(
        ...
        """Procesa un archivo de template individual (equivalente a processTemplate de TypeScript)"""
        try:
            # Leer contenido del template
            with open(template_path, 'r', encoding='utf-8') as f:
                template_content = f.read()
            
            # Compilar template con Jinja2 (equivalente a Handlebars.compile)
    ...
    def _process_filename(self, file_path: str, context: TemplateContext) -> str:
        """Procesa el nombre del archivo reemplazando variables (equivalente a processFileName de TypeScript)"""
        ...
        # Manejar casos especiales para páginas (equivalente a la lógica de TypeScript)
    ...
    def _get_file_type(self, file_path: str) -> str:
        """Determina el tipo de archivo basado en su ruta (equivalente a getFileType de TypeScript)"""
    ...
    async def _generate_readme(
        ...
        """Genera archivo README con documentación (equivalente a generateReadme de TypeScript)"""
    ...
    def _generate_readme_content(self, context: TemplateContext) -> str:
        """Genera el contenido del README (equivalente a generateReadmeContent de TypeScript)"""
...
"""
    ...
    async def validate_templates_path(self) -> bool:
        """Verifica la disponibilidad de templates (equivalente a validateTemplatesPath de TypeScript)"""
        try:
